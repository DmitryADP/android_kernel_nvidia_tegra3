...
static int kai_panel_enable(struct device *dev)
{
	...
	if (nabi2_dsi_reg == NULL && (machine_is_nabi2_xd() || machine_is_n1010()||machine_is_ns_14t004()||machine_is_itq1000()||
		machine_is_n1011() || machine_is_n1050())) {
		nabi2_dsi_reg = regulator_get(dev, "avdd_dsi_csi");
...
static struct platform_device nabi2_xd_dsi_disp1_device = {
	.name		= "tegradc",
	.id		= 0,
...
#ifdef CONFIG_BRIDGE_SSD2828
static struct platform_device nabi2_xd_disp1_device = {
	.name		= "tegradc",
	.id		= 0,
	.resource	= kai_disp1_resources,
	.num_resources	= ARRAY_SIZE(kai_disp1_resources),
...
#endif
static struct platform_device nabi2_3d_disp1_device = {
	.name		= "tegradc",
	.id		= 0,
	.resource	= kai_disp1_resources,
	.num_resources	= ARRAY_SIZE(kai_disp1_resources),
...
static struct platform_device nabi_2s_disp1_device = {
	.name		= "tegradc",
	.id		= 0,
	.resource	= kai_disp1_resources,
	.num_resources	= ARRAY_SIZE(kai_disp1_resources),
...
static struct platform_device nabi2_3d_disp2_device = {
	.name		= "tegradc",
	.id		= 1,
	.resource	= kai_disp2_resources,
	.num_resources	= ARRAY_SIZE(kai_disp2_resources),
...
static struct platform_device nabi_2s_disp2_device = {
	.name		= "tegradc",
	.id		= 1,
	.resource	= kai_disp2_resources,
	.num_resources	= ARRAY_SIZE(kai_disp2_resources),
...
if(machine_is_nabi2() || machine_is_nabi2_3d()||machine_is_nabi_2s()||machine_is_qc750() 
	...
	err = gpio_request(kai_lvds_vdd, "lvds_vdd_3v");
    if (err < 0) {
		pr_err("%s: gpio_request failed %d\n",
			__func__, err);
		return err;
	}
	err = gpio_direction_output(kai_lvds_vdd, 1);
    if (err < 0) {
		pr_err("%s: gpio_direction_output failed %d\n",
			__func__, err);
			gpio_free(kai_lvds_vdd);
		return err;
	}
    ...
    if (err < 0) {
		pr_err("%s: gpio_direction_itput failed %d\n",
			__func__, err);
	...
	if(machine_is_nabi2_3d()|| machine_is_qc750() ||  machine_is_n710() || machine_is_itq700() || machine_is_itq701() || machine_is_mm3201() || machine_is_n750() || machine_is_birch() || machine_is_wikipad())
	{
		res = platform_get_resource_byname(&nabi2_3d_disp1_device,
					 IORESOURCE_MEM, "fbmem");
	}
	else if(machine_is_nabi_2s())
	{
		res = platform_get_resource_byname(&nabi_2s_disp1_device,
					 IORESOURCE_MEM, "fbmem");
	}
#ifdef CONFIG_BRIDGE_SSD2828
	else if(machine_is_nabi2_xd() )
		res = platform_get_resource_byname(&nabi2_xd_disp1_device,
					 IORESOURCE_MEM, "fbmem");
#elif defined(CONFIG_KEENHI_NV_DC_DSI_OUT)
	else if(machine_is_nabi2_xd() || machine_is_n1010() || machine_is_ns_14t004() || machine_is_itq1000() || 
			machine_is_n1011() || machine_is_n1050())
		res = platform_get_resource_byname(&nabi2_xd_dsi_disp1_device,
					 IORESOURCE_MEM, "fbmem");
#endif
	else
	{
	...
	{
		if (!err){
		nabi2_3d_disp1_device.dev.parent = &phost1x->dev;
		err = platform_device_register(&nabi2_3d_disp1_device);
        }
		res = platform_get_resource_byname(&nabi2_3d_disp2_device,
						 IORESOURCE_MEM, "fbmem");
		res->start = tegra_fb2_start;
		res->end = tegra_fb2_start + tegra_fb2_size - 1;
		if (!err){
		nabi2_3d_disp2_device.dev.parent = &phost1x->dev;
			err = platform_device_register(&nabi2_3d_disp2_device);
	}
    }
	else if(machine_is_nabi_2s())
	{
		if (!err){
		nabi_2s_disp1_device.dev.parent = &phost1x->dev;
		err = platform_device_register(&nabi_2s_disp1_device);
        }
		res = platform_get_resource_byname(&nabi_2s_disp2_device,
						 IORESOURCE_MEM, "fbmem");
		res->start = tegra_fb2_start;
		res->end = tegra_fb2_start + tegra_fb2_size - 1;
		if (!err){
		nabi_2s_disp2_device.dev.parent = &phost1x->dev;
			err = platform_device_register(&nabi_2s_disp2_device);
	}
}
	else
	{
	...
	if(machine_is_nabi2_xd() ){
		if (!err){
		nabi2_xd_disp1_device.dev.parent = &phost1x->dev;
		err = platform_device_register(&nabi2_xd_disp1_device);
	}
}
    else
#elif defined(CONFIG_KEENHI_NV_DC_DSI_OUT)
	if(machine_is_nabi2_xd() || machine_is_n1010() || machine_is_ns_14t004() || machine_is_itq1000() ||
		machine_is_n1011() || machine_is_n1050()){
		if (!err){
		nabi2_xd_dsi_disp1_device.dev.parent = &phost1x->dev;
		err = platform_device_register(&nabi2_xd_dsi_disp1_device);
	}
