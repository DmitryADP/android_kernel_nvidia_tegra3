...
static struct i2c_board_info enterprise_i2c4_tmp401_board_info[] = {
	...
	//	.irq = gpio_to_irq(TEGRA_GPIO_TEMP_ALERT_N),
...
static int kai_gc0308_power_on(struct device *dev)
{
	...
	if (kai_1v8_cam1 == NULL) {
		kai_1v8_cam1 = regulator_get(dev, "vdd_1v8_cam1");
	...
	if (kai_vdd_cam1 == NULL) {
		kai_vdd_cam1 = regulator_get(dev, "vdd_cam1");//2.8v
...
static int kai_gc0308_power_off(struct device *dev)
{
	pr_err("%s:==========>EXE\n",__func__);
	gpio_direction_output(GC0308_POWER_DWN_PIN, 1);//old 1
	gpio_direction_output(GC0308_POWER_RST_PIN, 0);
...
static int kai_hm2057_power_on(struct device *dev)
{
	...
	if (kai_vdd_cam1 == NULL) {
		kai_vdd_cam1 = regulator_get(dev, "vdd_cam1");
		if (WARN_ON(IS_ERR(kai_vdd_cam1))) {
			pr_err("%s: couldn't get regulator vdd_cam1: %d\n",	__func__, (int)PTR_ERR(kai_vdd_cam1));
			goto reg_get_vdd_cam3_fail;
		}
...
static int kai_hm2057_power_off(struct device *dev)
{
	pr_err("%s:==========>EXE\n",__func__);
	gpio_direction_output(HM2057_POWER_DWN_PIN, 1);//old 1
	gpio_direction_output(HM2057_POWER_RST_PIN, 0);
...
static int kai_t8ev5_power_on(struct device *dev)
{
...
static int kai_t8ev5_power_off(struct device *dev)
{
	pr_err("%s:==========>EXE\n",__func__);	
	gpio_direction_output(T8EV5_POWER_DWN_PIN, 0);//old 1
	gpio_direction_output(T8EV5_POWER_RST_PIN, 0);
...
#ifdef CONFIG_LIS3DH_ACCELEROMETER
...
static void keenhi_gsensor_init(void)
{
	...
	if(machine_is_nabi2_xd()){
		nabi2_xd_inv_mpu_i2c0_board_info[0].irq = gpio_to_irq(DMARD06_IRQ_GPIO); // Replace with appropriate GPIO setup
		...
		nabi2_n710_inv_mpu_i2c0_board_info[0].irq = gpio_to_irq(DMARD06_IRQ_GPIO); // Replace with appropriate GPIO setup
		...
		nabi2_n1010_inv_mpu_i2c0_board_info[0].irq = gpio_to_irq(DMARD06_IRQ_GPIO); // Replace with appropriate GPIO setup
	...
	{
		nabi2_n750_inv_mpu_i2c0_board_info[0].irq = gpio_to_irq(DMARD06_IRQ_GPIO); // Replace with appropriate GPIO setup
		i2c_register_board_info(MPU_GYRO_BUS_NUM, nabi2_n750_inv_mpu_i2c0_board_info,
			ARRAY_SIZE(nabi2_n750_inv_mpu_i2c0_board_info));
	}
	else if(machine_is_ns_14t004()){
		nabi2_ns_14t004_inv_mpu_i2c0_board_info[0].irq = gpio_to_irq(DMARD06_IRQ_GPIO); // Replace with appropriate GPIO setup
		...
		nabi2_qc750_inv_mpu_i2c0_board_info[0].irq = gpio_to_irq(DMARD06_IRQ_GPIO); // Replace with appropriate GPIO setup
		...
		nabi_2s_inv_mpu_i2c0_board_info[0].irq = gpio_to_irq(DMARD06_IRQ_GPIO); // Replace with appropriate GPIO setup
...
#endif
    {
		inv_mpu_i2c0_board_info[0].irq = gpio_to_irq(DMARD06_IRQ_GPIO); // Replace with appropriate GPIO setup
#ifdef CONFIG_SENSORS_BMA250
		inv_mpu_i2c0_board_info[0].irq = gpio_to_irq(BMA250_IRQ_GPIO);
#endif
#ifdef CONFIG_SENSORS_DMARD06
		inv_mpu_i2c0_board_info[0].irq = gpio_to_irq(DMARD06_IRQ_GPIO);
#endif
...
static void keenhi_compass_init(void)
{
	...
	if(machine_is_nabi2_3d() || machine_is_nabi2_xd() ||machine_is_qc750() ||  machine_is_n710() || machine_is_itq700() || machine_is_itq701() || machine_is_mm3201()
		|| machine_is_n1010() || machine_is_n750() || machine_is_wikipad()||machine_is_ns_14t004()) {
			keenhi_i2c0_board_info_copmass[0].irq = gpio_to_irq(TEGRA_GPIO_PW0);
    ...
    if(machine_is_birch())
	{
        keenhi_i2c0_birch_copmass[0].irq = gpio_to_irq(TEGRA_GPIO_PW0);
...
int __init kai_sensors_init(void)
{
	...
	if (err)
        ...
        enterprise_i2c4_tmp401_board_info[0].irq = gpio_to_irq(KAI_TEMP_ALERT_GPIO);
		i2c_register_board_info(4, enterprise_i2c4_tmp401_board_info,
			ARRAY_SIZE(enterprise_i2c4_tmp401_board_info));
